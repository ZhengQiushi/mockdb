...
echo "$REGION_LEADERS" | jq -c '.[]' | while read -r region; do
  ...
  # 计算目标 store_id 的位置
  target_index=$(( (region_id + 1) % ${#GLOBAL_STORE_IDS[@]} ))
  target_store_id=${GLOBAL_STORE_IDS[$target_index]}
  ...
  # 执行添加 replica 操作
#   echo "Adding replica for region $region_id on store $target_store_id"
    echo "tiup ctl:v8.5.0 pd -u "$PD_ADDRESS" operator add add-peer $region_id $target_store_id"
#   tiup ctl:v8.5.0 pd -u "$PD_ADDRESS" operator add add-peer $region_id $target_store_id

  # 检查命令是否执行成功
#   if [ $? -eq 0 ]; then
#     echo "Successfully added replica for region $region_id on store $target_store_id"
#   else
#     echo "Failed to add replica for region $region_id on store $target_store_id"
#   fi
...
echo "Replica addition completed."